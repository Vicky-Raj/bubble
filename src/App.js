...
function App() {
  ...
  return (
    ...
    <header style={{textAlign:"center"}}>
      ...
      {running?
      <input type="range" min="0" max="200" step="1" value={count} onChange={(e)=>changeCount(e.target.value)} disabled/>
      :<input type="range" min="0" max="200" step="1" value={count} onChange={(e)=>changeCount(e.target.value)} />
      }
      <span>COUNT:{count}</span>
      <button onClick={()=>changeRunning(false)}>STOP</button>
    </header>
    <main  style={{width:`${WIDTH}px`,height:`${HEIGHT}px`,border:"1px solid red",margin:"20px auto"}}>
    {
      bars.map((height,index)=>{
        ...
        width={width} 
        count={count} 
        color={index === current[1] || index === current[1]+1 ? "brown" : "DodgerBlue"}
        key={height}
        />
      })
    }
